...
BOOL Memory::SetPrivilege(HANDLE hToken, LPCTSTR lpszPrivilege, BOOL bEnablePrivilege)
{
    ...
    if (!LookupPrivilegeValue(NULL, lpszPrivilege, &luid)) {
        //kLog("LookupPrivilegeValue error: %u\n", GetLastError());
        return FALSE;
    }
    ...
    if (!AdjustTokenPrivileges(hToken, FALSE, &tp, sizeof(TOKEN_PRIVILEGES), (PTOKEN_PRIVILEGES)NULL, (PDWORD)NULL)) {
        //kLog("AdjustTokenPrivileges error: %u\n", GetLastError());
        //return FALSE;
    }

    if (GetLastError() == ERROR_NOT_ALL_ASSIGNED) {
        //kLog("The token does not have the specified privilege. \n");
        //return FALSE;
    }
...
BOOL Memory::GetDebugPrivileges(void) {
    ...
    {
        //kLog("OpenProcessToken() failed, error\n>> %d", GetLastError());
        //return FALSE; //std::cout << "OpenProcessToken() failed, error\n>> " << GetLastError() << std::endl;
    }
    else //kLog("OpenProcessToken() is OK, got the handle!");

    if (!SetPrivilege(hToken, SE_DEBUG_NAME, TRUE))
    {
        //kLog("Failed to enable privilege, error:\n>> ", GetLastError());
        //return FALSE; 
    }
